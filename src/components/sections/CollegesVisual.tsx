...
const studentAvatars = [1, 2, 3, 4].map((n) => `/images/hero/avatar-fellow-${n}.jpg`);

/**
 * Right-side visual for the For Colleges header — a photo card with a
 * floating student-avatar + stat chip overlapping its bottom edge, same
 * treatment as ProfessionalsVisual, with copy specific to a campus
 * partnership instead of individual upskilling.
 */
export default function CollegesVisual() {
  return (
    <div className="relative mx-auto w-full max-w-[440px] pb-8 lg:mx-0 lg:ml-auto">
      <div className="relative overflow-hidden rounded-[28px] card-shadow-lg">
        <div className="relative h-[300px] w-full sm:h-[360px]">
          <Image
            src="/collageimage.jpg"
            alt="A student building real projects through a KodoWorks campus partnership"
            fill
            sizes="440px"
        ...
        <span className="absolute left-4 top-4 inline-flex items-center gap-2 rounded-full bg-white/95 px-3.5 py-1.5 text-[11px] font-semibold uppercase tracking-wide text-navy-800">
          ...
          Campus Talent Pipeline
        </span>
      </div>

      {/* Floating student-avatar + stat chip, overlapping the photo's bottom edge */}
      <div className="absolute -bottom-2 left-1/2 flex w-[92%] -translate-x-1/2 items-center justify-between gap-3 rounded-2xl bg-white px-4 py-3.5 card-shadow-lg sm:px-5">
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="text-[12.5px] font-bold leading-tight text-navy-950">7 Fellowship Programs</p>
            <p className="text-[10.5px] leading-tight text-navy-400">Run directly on your campus</p>
          </div>
        </div>
